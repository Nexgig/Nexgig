...
export default function AddSlotScreen() {
  ...
  // Guest DJ — one-time off-app performers (name-only bookings, booked immediately). Adding one
  // ensures the slot exists first (this screen creates the slot on demand).
  const slotGuests = createdSlotId ? allBookings.filter((b) => b.slotId === createdSlotId && !!b.guestName && b.status !== 'cancelled') : [];
    ...
    return (
      <Fragment key={artistId}>
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            <Text numberOfLines={1} style={[styles.artistName, { color: state === 'requested' ? colors.muted : colors.foreground }]}>{user.fullName}</Text>
        ...
        {isPast ? (
          ...
          <>
            ...
            {slotGuests.map((g) => (
              <Fragment key={g.id}>
                <Divider full />
                <Pressable style={({ pressed }) => [styles.artistRow, { opacity: pressed ? 0.6 : 1 }]} onPress={() => removeGuest(g)}>
...
const styles = StyleSheet.create({
  ...
  guestAvatar: { width: 42, height: 42, borderRadius: 21, alignItems: 'center', justifyContent: 'center' },
  guestInitial: { fontSize: 17, fontWeight: '800' },
  guestDashedAvatar: { width: 42, height: 42, borderRadius: 21, borderWidth: 1.5, borderStyle: 'dashed', alignItems: 'center', justifyContent: 'center' },
