                  ...
                  return (
                    ...
                    >
                      <View style={[
                        ...
                        <Text style={[styles.dayNumber, { color: isSelected ? colors.background : isToday ? colors.error : colors.foreground }]}>{day}</Text>
                      </View>
                      {dots.length > 0 && (
          ...
          <View style={[slotModalStyles.sheet, { backgroundColor: colors.background, height: slotModalHeight }]}>
            ...
            {!editingSlot && (
              <View style={slotModalStyles.fieldBlock}>
                <Text style={[slotModalStyles.fieldLabel, { color: colors.muted }]}>VENUE</Text>
                <ScrollView
                  horizontal
                  showsHorizontalScrollIndicator={false}
                  keyboardShouldPersistTaps="handled"
                  contentContainerStyle={slotModalStyles.pillRow}
                >
                  {venues.map((v) => {
                    ...
                    return (
                      ...
                      >
                        <Text style={[slotModalStyles.venuePillText, { color: sel ? '#fff' : colors.foreground }]} numberOfLines={1}>
                          {v.name}
                        </Text>
                      </Pressable>
                    );
            ...
            {/* ── Slot name ── */}
            <View style={slotModalStyles.fieldBlock}>
              <Text style={[slotModalStyles.fieldLabel, { color: colors.muted }]}>SET TIME</Text>
              {/* Quick preset chips */}
              <View style={slotModalStyles.presetRow}>
                {SLOT_PRESETS.map((preset) => (
                  ...
                  >
                    <Text style={[slotModalStyles.presetChipText, {
                      color: slotForm.name === preset.name ? colors.primary : colors.foreground,
                      fontWeight: slotForm.name === preset.name ? '700' : '500',
                    }]}>{preset.name}</Text>
                  </Pressable>
                ))}
              </View>
            ...
            <View style={slotModalStyles.timeRow}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <View style={{ position: 'relative', zIndex: startTimeOpen ? 20 : 1 }}>
                  <Pressable
                    style={[slotModalStyles.timeDropdownBtn, {
                      backgroundColor: colors.surface,
                      borderColor: startTimeOpen ? colors.primary : colors.border,
                    }]}
                  ...
                  >
                    ...
                    <Text style={[slotModalStyles.timeDropdownText, { color: colors.foreground }]}>
                      {slotForm.startTime}
                    </Text>
                    <MaterialIcons name={startTimeOpen ? 'keyboard-arrow-up' : 'keyboard-arrow-down'} size={18} color={colors.muted} />
                  </Pressable>
                  {startTimeOpen && (
                    <View style={[slotModalStyles.timeDropdownList, slotModalStyles.timeDropdownAbsolute, { backgroundColor: colors.background, borderColor: colors.border }]}>
                      <ScrollView ref={startTimeScrollRef} onLayout={() => scrollToTimeOption(startTimeScrollRef, slotForm.startTime)} style={slotModalStyles.timeDropdownScroll} nestedScrollEnabled showsVerticalScrollIndicator={false}>
                        {TIME_OPTIONS.map((t) => {
                          const isSelected = slotForm.startTime === t;
                          return (
                            <Pressable
                              key={t}
                              style={[slotModalStyles.timeOption, isSelected && { backgroundColor: colors.primary + '15' }]}
                              onPress={() => { setSlotForm((f) => ({ ...f, startTime: t })); setStartTimeOpen(false); }}
                            >
                              <Text style={[slotModalStyles.timeOptionText, {
              ...
              <View style={{ flex: 1 }}>
                ...
                <View style={{ position: 'relative', zIndex: endTimeOpen ? 20 : 1 }}>
                  <Pressable
                    style={[slotModalStyles.timeDropdownBtn, {
                      backgroundColor: colors.surface,
                      borderColor: endTimeOpen ? colors.primary : colors.border,
                    }]}
                  ...
                  {endTimeOpen && (
                    <View style={[slotModalStyles.timeDropdownList, slotModalStyles.timeDropdownAbsolute, { backgroundColor: colors.background, borderColor: colors.border }]}>
                      <ScrollView ref={endTimeScrollRef} onLayout={() => scrollToTimeOption(endTimeScrollRef, slotForm.endTime)} style={slotModalStyles.timeDropdownScroll} nestedScrollEnabled showsVerticalScrollIndicator={false}>
                        {TIME_OPTIONS.map((t) => {
                          ...
                          return (
                            <Pressable
                              key={t}
                              style={[slotModalStyles.timeOption, isSelected && { backgroundColor: colors.primary + '15' }]}
                              onPress={() => { setSlotForm((f) => ({ ...f, endTime: t })); setEndTimeOpen(false); }}
                            >
                              <Text style={[slotModalStyles.timeOptionText, {
                                color: isSelected ? colors.primary : colors.foreground,
                                fontWeight: isSelected ? '700' : '400',
            ...
            {/* ── CTA ── */}
            <View style={[slotModalStyles.ctaRow, { paddingBottom: Math.max(insets.bottom, 16), flexDirection: 'column', gap: 10 }]}>
              <Pressable
                style={({ pressed }) => [slotModalStyles.ctaBtn, { backgroundColor: colors.primary, opacity: pressed ? 0.85 : 1 }]}
                onPress={() => handleSaveSlot(!editingSlot)}
              >
                <MaterialIcons name={editingSlot ? 'check' : 'add'} size={18} color="#fff" />
                <Text style={slotModalStyles.ctaBtnText}>
                  {editingSlot ? 'Save Changes' : 'Assign Artist Now'}
                </Text>
              </Pressable>
              {!editingSlot && (
                <Pressable
