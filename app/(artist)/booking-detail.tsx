...
export default function DJBookingDetailScreen() {
  ...
  const notifyManager = (type: 'booking_confirmed' | 'booking_declined' | 'booking_cancelled') => {
    ...
    addNotification({
      ...
      type,
      title: titles[type],
      body: `${currentUser.fullName} at ${venueName} — ${date ? formatDate(date) : ''}`,
      isRead: false,
...
console.log('cancel sync called:', booking.id);
  ...
  const statusColors: Record<string, string> = {
    ...
    past_confirmation: colors.warning,
                    ...
                    onPress={() => {
                      ...
                      if (booking.managerId) {
                        addNotification({
                          ...
                          body: `${currentUser?.fullName ?? 'An artist'} left a ${reviewRating}-star review for ${booking.slotName || 'a gig'}`,
                          relatedId: booking.id,
                          relatedType: 'booking',
                          isRead: false,
                          createdAt: new Date().toISOString(),
