...
### POLISHING  (batch added 16 July 2026 — all JS-only → OTA unless noted)
  ...
  6. **All Bookings pages are now out of step with the rest.** 17 Jul: the dashboards and the
     confirmed/pending/completed pages all moved to the shared `DateBadge`, but
     `app/(artist)/all-bookings.tsx` still shows venue images (+ the generic event icon for
     private gigs, `:110`) and `app/(manager)/all-bookings.tsx` still renders `statusMark`
     status dots (`:134`). Same swap as the other lists — the odd ones out now.
...
- (Jul 8 2026, session 2) **Avatar system + manager-profile polish + invoice multi-select** (all JS-only → OTA). Worked in Cowork (files edited directly on disk; Tuts runs git/eas).
  ...
  • **Avatar picker** — new `components/ui/avatar-picker.tsx`: page-sheet grid of the 22 bundled avatars (`lib/avatars.ts`), coral ring on the selected. Added "Choose an Avatar" to both edit-profiles' photo action sheet (pick clears photo so avatar shows; photo kept as hidden fallback). Added an OPTIONAL Profile-Photo step to BOTH signups placed BEFORE the last page (upload photo first + avatar picker below): artist-setup 3→4 steps (before Media), manager-register 2→3 steps (before profile details). Persists `avatar_id` to the artists/managers rows (columns already added in Supabase; `users` NOT needed — sign-in hydrates from role tables). Hydrated at sign-in + artist profile refetch; mapped `avatar_id→avatarId` in network/assign-artist/profile-view/_layout loaders so chosen avatars show on everyone's cards. [PUSHED]
  • **2 avatar bug-fixes** — artist-side `artist-profile-view` + manager profile tab were rendering `<AvatarImage>` without `avatarId`/`seed`, so they showed a name-seeded default that mismatched the id-seeded list cards (and never showed a chosen avatar). Fixed both (map avatar_id + pass avatarId + seed={id}). [PUSHED]
  • **Manager profile polish** — stat numbers coral→black (coral `+` empty-state kept); invoice unread badge orange→red (`colors.error`). [PENDING its own commit/push — in working tree]
  • **Invoice multi-select** — replaced "Download All" with iOS edit-mode: "Select" enters select mode → inline checkboxes on each invoice, "Select All / Deselect All" row with live count, header shows a coral **download** icon + a red **delete** icon (both enabled ≥1) + "Cancel". Download zips the chosen invoices to the native share sheet; Delete removes them (confirm alert), both exit select mode after. REMOVED the old swipe-left-to-delete entirely — deletion now happens only via select mode. In `app/(manager)/(tabs)/profile.tsx`. [PENDING its own commit/push — in working tree]
- (Jul 8 2026) **Clean native cold-start launch** (OTA). Long debugging saga — first attempts (splash cross-fade, themed ThemeProvider bg, tab sceneStyle, min-splash timing, theme-hydration gate) were REVERTED to baseline `ee28f34` because they chased the wrong layer. ROOT CAUSE: the app anchored on the `(tabs)` group, whose `_layout.tsx` was a **Tabs navigator with a visible "Home" tab** (the "home button in the middle" screen); its only child `index.tsx` was a redirect gate that fired an ANIMATED `<Redirect>` to the real destination — so every cold start showed: splash → home-tab screen → SLIDE (modal slide-up to Welcome / push slide-from-right to dashboard). Compounded by `useAuthStore` being async-persisted (signed-in users looked signed-out on frame 1). FIX (3 files): (1) `app/(tabs)/_layout.tsx` Tabs navigator → plain `<Slot />` (no tab bar, nothing to flash); (2) `app/(tabs)/index.tsx` waits for `useAuthStore.persist.hasHydrated()` before redirecting (renders blank themed View meanwhile), routes exactly once; (3) `app/_layout.tsx` — hold splash until fonts AND auth hydrated, root Stack `animation: "none"` (+ removed `(auth)` fullScreenModal so no slide-up), and hide splash after a **double requestAnimationFrame** so the destination completes its first layout pass BEHIND the splash (kills the small content "settle"/jump on entry that the old slide used to mask). Net: coral splash → straight onto Welcome or dashboard, no home-tab, no slide, iOS+Android. Each step a single revertable commit. [If a residual Welcome settle remains, next lever is its `paddingTop: height*0.16` from `useWindowDimensions` → stabilize via initialWindowMetrics.]
...
_  • PERFORMER LABEL everywhere: new `performerLabel(instruments)` in `lib/utils.ts` ('CDJ / Turntables' → DJ, else Musician, empty → 'Artist' legacy fallback). Replaces the genre subtitle under the artist name on ALL cards: My Artists, manager+artist Network, both artist-profile-views, assign-artist (2 lists), artist own profile. Pending 'Wants to join' + genre search filters untouched._
_  • SIGNUP WIZARD (`app/(auth)/artist-setup.tsx`): performer choice moved to step 1 + made MANDATORY; Bio removed from signup (still in edit-profile); 4 → 3 steps; media step Instagram-first; Continue button pinned as keyboard-aware footer._
_  • AUTO-USERNAME: username field/validation removed from signup; auto-generated from artist name (dj_kai → dj_kai2…), pre-check + unique-violation retry backstop. Fixes the old USERNAME-TAKEN raw-Postgres-error bug._
_  • AUTH BACK: `choose-account-type.tsx` got a back button (→ welcome, canGoBack fallback) so an email signup can switch to Apple/Google._
_  • VENUE-FORM PILLS: create-venue + edit-venue chips squared from borderRadius 20 → 6 to match the setup-wizard rectangular pills (app consistency). create-venue Continue button made keyboard-aware (was already pinned)._
_)_
...
- EMAIL CONFIRMATION for email/password signups (PARKED July 1 2026 — revisit at public App Store launch, or sooner if junk artist signups start polluting the public directory). Goal: confirmation email whose link opens the app via the `manusapp://` deep-link scheme (same scheme Apple/Google OAuth already uses). Requires: (SERVER, Tuts) Supabase Auth → enable 'Confirm email' + allow-list the `manusapp://` redirect + optional template copy. (APP, code) pass `emailRedirectTo: 'manusapp://<callback>'` on email signUp; since NO session exists until confirmed, stop creating the profile / routing to dashboard right after signUp — stash the pending form (AsyncStorage) + show a 'check your inbox' screen; extend the deep-link/callback handler (reuse oauth/callback + _layout cold-start handling) to exchange the token → read stash → create users/artists rows (username auto-gen MOVES here, needs the authed session) → route to dashboard; handle cold-start + a resend option. NOTE: with confirm-email OFF (current), the post-signUp block runs authenticated so today's username-gen works; enabling confirm-email breaks that until the block is moved post-confirmation.
