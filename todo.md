...
- (Jul 8 2026, session 2) **Avatar system + manager-profile polish + invoice multi-select** (all JS-only → OTA). Worked in Cowork (files edited directly on disk; Tuts runs git/eas).
  ...
  • **Invoice multi-select** — replaced "Download All" with iOS edit-mode: "Select" enters select mode → inline checkboxes on each invoice, "Select All / Deselect All" row with live count, header "Download (N)" (coral, enabled ≥1) + "Cancel"; downloads only the chosen invoices as a zip to the native share sheet, then exits. Swipe-to-delete kept in normal mode, disabled while selecting. In `app/(manager)/(tabs)/profile.tsx`. [PENDING its own commit/push — in working tree]
