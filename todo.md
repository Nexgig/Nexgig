...
_(Report flow email wiring — DONE June 25 2026, ships via OTA: the flag/report on venue + artist pages (ReportModal) already inserted into the Supabase `reports` table, but that insert is wrapped in a silent try/catch AND there was no notification to admin — so reports were invisible unless you queried the DB (and silently lost if the table/RLS was wrong). Wired it to ALSO email admin@nexgigapp.com via `mailto:` (same mechanism as Send Feedback): opens the reporter's mail app pre-filled with type/name/id/reason/details/reporter-id. Kept the Supabase insert AND the silent 'Report submitted' confirmation (per request — no error surfaced either way). Single client-side file (`components/report-modal.tsx`) → OTA. ⚠ STILL UNVERIFIED: whether the Supabase `reports` table exists + has an authenticated INSERT RLS policy — if not, the DB row is silently lost but the email still arrives. To verify: `select count(*) from public.reports;` in Supabase; if it errors 'relation does not exist', the table was never created.)_
