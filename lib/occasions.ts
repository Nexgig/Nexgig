...
export const OCCASIONS: Occasion[] = [
  { key: 'club',      label: 'Club',      icon: 'nightlife' },
 ...
 * so they fall back to the neutral 'event' calendar glyph — NOT the default 'club', because a
 * legacy event isn't necessarily a club night.
 */
export function occasionIcon(occasion?: string | null): IconName {
  return (occasion && BY_KEY[occasion]?.icon) || 'event';
}
